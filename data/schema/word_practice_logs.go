...
// Each row is an append-only record of a single quiz answer's familiarity
// change, so familiarity trends, practice streaks, and daily volume can be
// derived later without touching the words table's own running counters.
func WordPracticeLogsTable() *domain.TableDefinition {
	return &domain.TableDefinition{
		...
		Description: "Append-only log of each quiz answer's familiarity change for a word",
	}
}
