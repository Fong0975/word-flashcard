...
describe('useWords', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(apiService, 'getWordsCount').mockResolvedValue({ count: 0 });
  });
  ...
  it('searches across word, definition, and notes with OR logic for a plain keyword', async () => {
    ...
    const params = lastCallArg(apiService.searchWords as Mock);
  ...
  it('mirrors entities/fetchEntities as words/fetchWords', async () => {
    ...
    (apiService.searchWords as Mock).mockResolvedValue([word]);
    (apiService.getWordsCount as Mock).mockResolvedValue({ count: 1 });
