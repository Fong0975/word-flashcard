...
export const LoadingButton: React.FC<LoadingButtonProps> = ({
  ...
  return (
    <button
      {...props}
      disabled={isDisabled}
      className={` ${getSizeClasses()} ${getVariantClasses()} ${fullWidth ? 'w-full' : ''} ${className} flex items-center justify-center`.trim()}
    >
      {isLoading ? (
        <>
          <svg
            className={`-ml-1 mr-2 animate-spin ${getSpinnerSize()} text-current`}
            fill='none'
            viewBox='0 0 24 24'
          >
            <circle
